<%@ page import="com.utils.SQL_Operations" %>
...
<div class="container">
    ...
    <%
        ...
        List<String> list = sql_operations.getAllColumnsNamesFromTable("student");
    %>
    ...
    <table id="myTable" border="1" class="table">
        ...
        <tr>
            <% // выводим имена колонок
                for (String item : list) out.println("<th scope=\"col\">" + item + "</th>");
            %>
        </tr>
        </thead>
        ...
        <tbody>
        <%
            try (ResultSet resultSet = sql_operations.getResultSetByTable("list")) {
            ...
            <%
                for (int i = 0; i < list.size(); i++) {
                    if (i == 0) {
                        ...
                        out.print(resultSet.getString(list.get(i)));
                        out.print("</th>");
                    } else {
                        out.print("<td>");
                        out.print(resultSet.getString(list.get(i)));
    ...
    <form name="forwardForm" action="${pageContext.request.contextPath}/DeleteColumns">
        ...
        <select name="columnName">
            <% for (int i = 1; i < list.size(); i++) {
                out.print("<option value=\"" + list.get(i) + "\" > ");
        ...
        <table id=" myTable1">
            ...
            <tr>
                <% // id - то не стоит изменять..
                    for (int k = 1; k < list.size(); k++) out.println("<th>" + list.get(k) + "</th>");
                %>
            </tr>
            <tr>
                <% // id сам инкрементится
                    for (int i = 1; i < list.size(); i++) {
                        out.print("<td>");
                        if (i == 3)
                            out.print("<input name=\"" + list.get(i) + "\" type=\"number\" class=\"ol-md-3 mb-3 form-control\">");
                        else
                            out.print("<input name=\"" + list.get(i) + "\" type=\"text\" class=\"ol-md-3 mb-3 form-control\">");
                        out.print("</td>");
                    }
